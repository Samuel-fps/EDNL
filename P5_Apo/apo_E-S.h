...
template <typename T>
void rellenarApo(Apo<T>& A, const T& fin)
// Pre: A está vacío.
...
template <typename T>
void rellenarDescendientes(typename Apo<T>::nodo n, Apo<T> &A, const T &fin)
...
//       usando fin para introducir nodos nulos.
{
   ...
   assert(A.hijoIzqdo(n) == Apo<T>::NODO_NULO &&
          A.hijoDrcho(n) == Apo<T>::NODO_NULO);
...
template <typename T>
void rellenarApo(istream& is, Apo<T>& A)
// Pre: A está vacío.
...
template <typename T>
void rellenarDescendientes(istream& is, typename Apo<T>::nodo n, Apo<T>& A, const T& fin)
// Pre: n es un nodo hoja de A.
...
template <typename T>
void imprimirDescendientes (typename Apo<T>::nodo n, const Apo<T>& A)
// Post: Muestra los descendientes de n en la salida estándar.
{
   if (A.hijoIzqdo(n) != Apo<T>::NODO_NULO)
   {
      cout << "Hijo izqdo de " << A.elemento(n) << ": "
                << A.elemento(A.hijoIzqdo(n)) << endl;
      imprimirDescendientes(A.hijoIzqdo(n), A);
   }
   if (A.hijoDrcho(n) != Apo<T>::NODO_NULO)
   {
...
template <typename T>
void imprimirApo (ostream& os, const Apo<T>& A, const T& fin)
...
template <typename T>
void imprimirDescendientes (ostream& os, typename Apo<T>::nodo n, const Apo<T>& A, const T& fin)
