...
//       usando fin para introducir nodos nulos.
{
   ...
   assert(A.hijoIzqdo(n) == Abb<T>::NODO_NULO &&
          A.hijoDrcho(n) == Abb<T>::NODO_NULO);
...
template <typename T>
void rellenarAbb(istream& is, Abb<T>& A)
// Pre: A está vacío.
...
template <typename T>
void rellenarDescendientes(istream& is, typename Abb<T>::nodo n, Abb<T>& A, const T& fin)
// Pre: n es un nodo hoja de A.
...
template <typename T>
void imprimirDescendientes (typename Abb<T>::nodo n, const Abb<T>& A)
// Post: Muestra los descendientes de n en la salida estándar.
{
   if (A.hijoIzqdo(n) != Abb<T>::NODO_NULO)
   {
      cout << "Hijo izqdo de " << A.elemento(n) << ": "
                << A.elemento(A.hijoIzqdo(n)) << endl;
      imprimirDescendientes(A.hijoIzqdo(n), A);
   }
   if (A.hijoDrcho(n) != Abb<T>::NODO_NULO)
   {
...
template <typename T>
void imprimirAbb (ostream& os, const Abb<T>& A, const T& fin)
...
template <typename T>
void imprimirDescendientes (ostream& os, typename Abb<T>::nodo n, const Abb<T>& A, const T& fin)
